...
**CORRECTION — résumé text IS searchable** (an earlier claim here that it wasn't was WRONG): `description` = the candidate's full parsed résumé text and is fully searchable (`description:clearance`≈39,816, `description:"Top Secret"`≈5,178). Also searchable: `comments` (~7,539 for clearance), `occupation` (~39), `skillSet` (sparse). NOT useful (empty/associations): primarySkills, secondarySkills, certifications, certificationList, educations, workHistories.
**Lesson / FIX SHIPPED:** `search_candidates` now takes `keywords: Array<string | string[]>`. `buildCandidateKeywordQuery` (bullhorn-client.ts) expands each term into a quoted, Lucene-escaped OR across `description`+`skillSet`+`comments`+`occupation`; top-level entries AND-ed (required concepts), inner arrays OR-ed (synonyms); then AND-combined with the structured `query`. This guarantees full text coverage and never emits a fieldless term, so it sidesteps BOTH traps above. Raw `query` stays for structured filters only; empty (no query AND no keywords) throws a clear error. Prefer `keywords` for any résumé/skills/clearance text search.

## Security clearance lives in résumé/notes TEXT, not a structured field
No dedicated clearance field exists (267 Candidate fields, none clearance/security). Clearance is ABUNDANT in résumé text (`description:clearance`≈39,816, `description:"Top Secret"`≈5,178) and notes (`comments`≈7,539), but only DOZENS in the structured `skillSet` tag (`skillSet:Secret`≈13, `skillSet:Clearance`≈18). So judge clearance by TEXT search (use the `keywords` arg), not a structured field.
Caveat: search `total` is relevance-approximate, not an exact census — confirm shortlisted candidates with get_candidate_resume, and remember a résumé mention ≠ a currently-active clearance (they lapse).
...
## "Blocked by the connector's safety checks" = ChatGPT's OWN privacy/safety layer, NOT our server or a size cap
When ChatGPT narrates that a Bullhorn query/résumé-read was "blocked by the connector's safety checks" (and guesses it's "because the terms are sensitive"), that is the ChatGPT client withholding the tool RESULT, not our server failing. Verified: re-running the exact calls server-side succeeds and returns SMALL payloads — broad grouped clearance+Java keyword search ≈28 KB (total≈19,323 matches), a full résumé read ≈26–38 KB, notes <1 KB. None approach a response-size limit. **Why it happens:** ChatGPT (esp. connector/deep-research mode) resists surfacing bulk PII / "dossiers on many real people" and dense personal data (full résumés w/ names, emails, phones tied to security-clearance attributes). That's why **narrowing the search "unblocks" it** — fewer named individuals trips the privacy guardrail less, not because the broad query errored.
**How to apply / mitigations (server-side levers only reduce, don't disable, ChatGPT's guardrail):** prefer returning a short EXCERPT around the matched keyword over the whole résumé; mask email/phone (we already strip SSN) to lower PII density; keep shortlists small; or compile server-side and hand ChatGPT a compact summary. Separately, bare `"Secret"` has a PRECISION problem (matches "Secret Manager"/AWS Secrets Manager) — steer to specific phrases like `"Secret clearance"`, `"Federal Government Secret"`.
