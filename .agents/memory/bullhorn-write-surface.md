...
- **Tearsheet membership** = to-many association endpoint with NO body: `PUT|DELETE entity/Tearsheet/{id}/candidates/{comma,ids}`. Bullhorn uses **PUT to ASSOCIATE** and DELETE to disassociate (POST on this path is entity-update and fails). `writeFetch(...,undefined)` sends no body — correct. Alternate bulk path exists (`POST massUpdate/Tearsheet` with `{candidates:{add:[…]},ids:[tearsheetId]}`) but is not required for the MCP tools.
