...
- `artifacts/api-server/src/lib/mcp-server.ts` — MCP server factory: 33 read tools + 29 Bullhorn write tools (writes via resolveWriteSession/runWriteTool, never cached, 403 → permission_denied). Writes are MCP-connector-only — never exposed on the REST /api/v1 or public OpenAPI surface
...
Recruiters enable the Bullhorn connector in their AI (ChatGPT, Claude, Gemini, or Grok) and can immediately ask: "Find candidates in Chicago with .NET experience", "What are the open jobs at Acme Corp?", "Submit candidate #123 to job #456 and add a note." 33 read tools span dedicated entity tools (candidates, jobs, companies, contacts, submissions, placements, notes, leads, opportunities, appointments, tasks, users), reporting tools, and generic fallbacks (search_entity, query_entity, get_entity, describe_entity). 29 write tools cover submissions and status changes, client submissions (sendouts — the "Client Submission" pipeline stage), notes, candidate edits (update_candidate) plus job/company/contact create+update, lead create+update and opportunity create+update (sales/BD parity), tasks and appointments (create + update/reschedule/complete), multi-user notifications (notify_users — assigns a reminder Task to the named users, since Bullhorn's in-app notification bell is a private internal API closed to all integrations), tearsheet curation, placements, and file/résumé upload with new-candidate creation — each under the recruiter's own OAuth session.
