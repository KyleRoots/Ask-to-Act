...
- `artifacts/api-server/src/lib/mcp-server.ts` — MCP server factory: 33 read tools + 22 Bullhorn write tools (writes via resolveWriteSession/runWriteTool, never cached, 403 → permission_denied). Writes are MCP-connector-only — never exposed on the REST /api/v1 or public OpenAPI surface
