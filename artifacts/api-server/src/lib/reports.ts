...
export const REPORTS_CATALOG = [
  ...
  },
  {
    ...
    parameters: {
      ...
      openJobsOnly: "optional boolean; default true — leave default for normal asks",
      applicantPool: "'responses' (default) or 'all' JobSubmission rows on those jobs",
      limit: "for 'N most recent' / 'list N' — set to N (max 50)",
      mode: "'bounded' (default) for list/most-recent; 'exhaustive' for lookback counts only",
      maxJobs: "optional — server auto-pages; do not ask the user",
      maxCandidatesToScan: "optional per page — do not ask the user",
      dateAddedStart: "optional YYYY-MM-DD (lookback counts)",
      dateAddedEnd: "optional YYYY-MM-DD exclusive (lookback counts)",
    },
  },
] as const;
