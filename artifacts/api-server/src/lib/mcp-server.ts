...
export function createMcpServer(): McpServer {
  ...
  tool(
    "count_entity",
    `Count Bullhorn records for a Lucene query WITHOUT returning the records — and optionally break the count down by a field. USE THIS for "how many" and scorecard / by-department questions instead of fetching records and counting them yourself: record lists cap at 100-500 and will undercount (e.g. report "51+" instead of the true total). Returns exact totals straight from Bullhorn in a tiny payload. Set \`groupBy\` to a field to get a per-value breakdown; for EXACT grouping also pass the known values in \`groupValues\` (e.g. the Internal Department names) — without them, values are auto-discovered from a sample and may be incomplete (groupsComplete:false). "Internal Department" field by entity: JobOrder & Placement = \`correlatedCustomText1\`; Opportunity, ClientContact & Lead = \`customText1\`; Candidate = \`customText3\`. For JobOrder, "open jobs" means EXACTLY \`isOpen:true AND NOT status:Archive\` (the open flag still includes on-hold/filled/placed; only Archived is excluded) — do NOT add extra status exclusions (Filled/Placed/Lost/Canceled/Declined/etc.) unless the user explicitly asks. For Placement, "placements made" / "placements so far" means CONFIRMED placements only — count \`(status:Approved OR status:Completed OR status:Ended)\` (i.e. exclude Canceled, Archive, AND pending Submitted) unless the user explicitly asks for all/pending/canceled placements. For Opportunity, "active" / "open" / "in the pipeline" opportunities means EXACTLY \`isOpen:true\` — the server then applies the official exclusion of Closed-Won/Closed-Lost/Converted; do NOT approximate "active" by enumerating a subset of statuses (e.g. \`status:Open OR status:Qualifying\`), which UNDERCOUNTS (it drops Qualified/New) — use \`isOpen:true\`, the sales_pipeline_report, or the staffing_scorecard. Status spellings are exact and case-sensitive (\`Canceled\` one l, \`Archive\` not "Archived"); a wrong spelling is silently ignored, so groupBy \`status\` to discover the valid values. Searchable entities only: Candidate, ClientContact, ClientCorporation, JobOrder, JobSubmission, Placement, Lead, Opportunity, Note.`,
    {
      entityType: z.string().describe(entityTypeDescribe),
      query: z
        .string()
