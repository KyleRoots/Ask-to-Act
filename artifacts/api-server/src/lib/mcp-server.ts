...
export function createMcpServer(caller?: CallerIdentity): McpServer {
  ...
  tool(
    "get_notes",
    "Retrieve notes and activity log entries for a candidate or job order, optionally within a dateAdded range. Provide at least one of candidateId or jobId. Uses Bullhorn structured /query (not Lucene search): candidate notes match personReference OR the candidates association; job notes match jobOrder.id. Results are in `data`. Raise `count` (max 50) or page with `start` for more. To filter by Note.action (including custom values like 'Scout Screen - Qualified' that may be absent from list_field_options), use query_entity(Note, where: \"action='…'\") instead.",
    {
  ...
  tool(
    ...
    "Returns the EXACT valid dropdown values configured in THIS Bullhorn instance for picklist fields (e.g. Note action types, Candidate status, JobSubmission status). " +
      ...
      "(e.g. 'Scout Screen - Qualified') that do not appear in list_field_options. Never reject a user-supplied Note.action when filtering/querying notes solely because it is missing from this list; use query_entity with the exact action string. " +
