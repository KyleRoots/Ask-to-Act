...
## Available Tools (33 Read + 34 Write)
...
**Write — 34 Bullhorn write tools** (per-user; each action runs as the recruiter's own Bullhorn identity)
...
- Workflow: `create_task`, `update_task`, `create_appointment`, `update_appointment`, `notify_users`, `create_tearsheet`, `add_candidates_to_tearsheet`, `remove_candidates_from_tearsheet`, `add_records_to_tearsheet`, `remove_records_from_tearsheet`
...
2. Set write permissions to **Enabled** (the 34 write tools require per-user Bullhorn enrollment; each recruiter writes under their own Bullhorn identity)
...
- **v2 — Write tools (live)**: 34 write tools for notes, submissions, placements, jobs, companies, tasks, tearsheets (multi-entity membership), file uploads, and permission-aware soft-delete/restore/archive
